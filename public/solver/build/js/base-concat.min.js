...
/*!
 * Generated using the Bootstrap Customizer (https://getbootstrap.com/customize/?id=4bf75079988b010b2615ec70046b8ee7)
 * Config saved to config.json and https://gist.github.com/4bf75079988b010b2615ec70046b8ee7
 */
...
function splitUrl(a){a||(a=window.location.href),a.lastIndexOf("#")==a.length-1&&(a=a.substring(0,a.length-1));var b=a.split("?");if(b.length<2)return{baseUrl:a,params:{}};var c=b[0],d=b[1];b=d.split("&");for(var e={},f=0;f<b.length;f++){var g=b[f].split("=");e[g[0]]=g[1]||""}return{baseUrl:c,params:e}}function composeUrl(a,b){a||(a=window.location.href);var c=splitUrl(a);a=c.baseUrl;var d=c.params;for(var e in b)d[e]=b[e];b=d;var f="";for(e in b)null!=b[e]&&(f+="&"+e+"="+b[e]);return 0==f.indexOf("&")&&(f=f.substring(1),f="?"+f),$.trim(a+f+" ")}function combineUrls(a,b){var c=splitUrl(b),d=c.baseUrl,e=c.params;c=splitUrl(a);var f=c.params;for(var g in e)f[g]=e[g];return composeUrl(d,f)}function absUrl(a){var b=document.createElement("a");return b.href=a||"",b.href}function sortList(a){var b=splitUrl();b.params.sort=b.params.sort==a&&0!=b.params.sort.indexOf("-")?"-"+a:a,window.location.href=composeUrl(b.baseUrl,b.params)}function setListPage(a){var b=splitUrl();b.params.page=a,window.location.href=composeUrl(b.baseUrl,b.params)}function alertTutors(a){confirm("Pay 99 cents to notify all relevant tutors that your question is open for answering. Receive a faster response from tutors to get your answer sooner!")&&(showCheckProgress(a),$.ajax({url:"/question/"+a+"/alerttutors/",type:"POST",success:function(b){"string"==typeof b&&(b=$.parseJSON(b),alert(b.results)),null!=b.error?showCheckError(a,b.error):showCheckResults(a,b.results)},error:function(){showCheckError(a)}}))}function hideQuestion(a){confirm("If you really don't want this question or any answers to this question to exist anymore you can delete it. It costs 1.99$ to delete a question and it will remove the question and any associated answers from search engines and from being shown on School Solver ever again. ")&&(showCheckProgress(a),$.ajax({url:"/question/"+a+"/hide/",type:"POST",success:function(b){"string"==typeof b&&(b=$.parseJSON(b),alert(b.results)),null!=b.error?showCheckError(a,b.error):showCheckResults(a,b.results)},error:function(){showCheckError(a)}}))}function isElementInViewport(a){"function"==typeof jQuery&&a instanceof jQuery&&(a=a[0]);var b=a.getBoundingClientRect();return b.top>=0&&b.left>=0&&b.bottom<=(window.innerHeight||document.documentElement.clientHeight)&&b.right<=(window.innerWidth||document.documentElement.clientWidth)}function readFullStory(){var a=$("#question_details");if(result=isElementInViewport(a),result===!1){var b=window.innerHeight;$(".question_content").wrap("<div class='read_full_wrapper'></div>");var c=$(".read_full_wrapper"),d=b-c.offset().top-5<100?100:b-c.offset().top-5;c.css({height:d,position:"relative",overflow:"hidden"});var e=$("<div></div>").addClass("read-full-btn-wrapper"),f=$("<div></div>").addClass("read-full-bg"),g=$("<button>Read Full Question</button>");g.css({padding:"10px 25px",backgroundColor:"white",color:"#2A7DB6",border:"1px solid #E5E5E5",textAlign:"center",margin:"30px auto",fontWeight:"Bold",fontSize:"15px",position:"absolute",zIndex:2}),f.css({width:"100%",height:"100%",position:"absolute",zIndex:1,opacity:.9}),e.css({width:"100%",height:"100px",position:"absolute",bottom:"0px"}),e.append(f),e.append(g),c.append(e),g.css({left:(e.outerWidth()-g.outerWidth())/2}),g.click(function(){f.remove(),g.remove(),e.remove(),c.css({height:"auto"})}),$("head").prepend("<style> .read-full-bg {background: -moz-linear-gradient(top,  rgba(255,255,255,0) 0%, rgba(255,255,255,1) 100%); /* FF3.6-15 */ background: -webkit-linear-gradient(top,  rgba(255,255,255,0) 0%,rgba(255,255,255,1) 100%); /* Chrome10-25,Safari5.1-6 */ background: linear-gradient(to bottom,  rgba(255,255,255,0) 0%,rgba(255,255,255,1) 100%); /* W3C, IE10+, FF16+, Chrome26+, Opera12+, Safari7+ */ filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#00ffffff', endColorstr='#ffffff',GradientType=0 ); /* IE6-9 */ } </style>")}}$(document).ready(function(){function a(a){var b=null;if(document.cookie&&""!=document.cookie)for(var c=document.cookie.split(";"),d=0;d<c.length;d++){var e=jQuery.trim(c[d]);if(e.substring(0,a.length+1)==a+"="){b=decodeURIComponent(e.substring(a.length+1));break}}return b}var b=a("csrftoken");$.ajaxSetup({crossDomain:!0,beforeSend:function(a,c){"POST"==c.type&&"/usertrack/userTrack/helpers/clearPartial.php"!=c.url&&a.setRequestHeader("X-CSRFToken",b)}})});
